...
#[link_section = ".git-commit"]
pub static GIT_COMMIT: [u8; 40] = *b"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
...
struct TakenState<'a, S, F>
where
    F: FnOnce(S) -> participant::borrower::State,
{
    inner: Option<TakenStateInner<'a, S, F>>,
}

impl<'a, S, F> TakenState<'a, S, F>
where
    F: Fn(S) -> participant::borrower::State,
{
    fn new(state: S, set: &'a mut Option<participant::borrower::State>, map: F) -> Self {
        TakenState {
            inner: Some(TakenStateInner { state, set, map }),
        }
    }

    fn try_map<E, F2>(mut self, map: F2) -> Result<(), E>
    where
        F2: FnOnce(S) -> Result<participant::borrower::State, (S, E)>,
    {
        let mut inner = self
            .inner
            .take()
            .expect("Attempt to map after successful transfer");
...
impl<'a, S, F> Drop for TakenState<'a, S, F>
where
    F: FnOnce(S) -> participant::borrower::State,
{
    fn drop(&mut self) {
...
impl Borrower {
    ...
    pub fn message_received(&mut self, message: &str) -> Result<(), JsValue> {
        ...
        match self.state.take().expect("use of invalidated Borrower") {
            participant::borrower::State::WaitingForFunding(state) => {
                let state = TakenState::new(
                    state,
                    &mut self.state,
                    participant::borrower::State::WaitingForFunding,
                );
                let hints = contract::offer::EscrowHints::deserialize(&mut &*bytes)
                    .map_err(into_debug_string)?;
                let cancel_fee_rate =
                    bitcoin::FeeRate::from_sat_per_vb(50 + hints.fee_rate.to_sat_per_vb_ceil())
                        .unwrap();
                ...
                let cancel_tx = state
                    .state()
                    .funding_cancel(txs, cancel_fee_rate, height, delay)
                    .map_err(into_debug_string)?;
                self.cancel_tx = Some(cancel_tx);
                state
                    .try_map(|state| {
                        state.funding_received(funding, &mut response).map(|state| {
                            participant::borrower::State::ReceivingEscrowSignature {
                                state,
                                received: None,
                            }
                        })
            ...
            participant::borrower::State::ReceivingEscrowSignature { state, received } => {
                let mut state =
                    TakenState::new((state, received), &mut self.state, |(state, received)| {
                        participant::borrower::State::ReceivingEscrowSignature { state, received }
                    });
                ...
                match (received.take(), message) {
                    ...
                    (Some(old @ TedSignatures::TedO(_)), TedSignatures::TedO(_))
                    | (Some(old @ TedSignatures::TedP(_)), TedSignatures::TedP(_)) => {
                        *received = Some(old);
        ...
        match self.state.take().expect("use of invalid state") {
            participant::borrower::State::SignaturesVerified(state) => {
                let state = TakenState::new(
                    state,
                    &mut self.state,
                    participant::borrower::State::SignaturesVerified,
                );
                let mut message = Vec::new();
                state
                    .try_map(|state| {
    ...
    pub fn compute_prefund_invoice(&self, reserve_sats: u64) -> Invoice {
        let (address, liq_amount) =
            match &self.state.as_ref().expect("attempt to use invalid state") {
                participant::borrower::State::WaitingForFunding(state) => {
                    (state.funding_address(), state.liquidator_amount())
                }
    ...
    pub fn serialize_state(&self) -> String {
        ...
        self.state
            .as_ref()
            .expect("attempt to use invalid state")
            .serialize(&mut buf);
        base64::encode(&buf)
    }
    ...
    pub fn deserialize_state(state: &str) -> Result<Borrower, JsValue> {
        ...
        let state =
            participant::borrower::State::deserialize(&mut &*bytes).map_err(into_debug_string)?;
        Ok(Borrower {
            state: Some(state),
            message: None,
            cancel_tx: None,
    ...
    pub fn debug_string_with_private_keys(&self) -> String {
        match self.state.as_ref() {
            Some(state) if state.network() != bitcoin::Network::Regtest => {
                panic!("debugging would leak private keys")
            }
            _ => format!("{:?}", self),
        }
    }
    ...
    pub fn state(&self) -> BorrowerState {
        match self.state.as_ref().expect("use of invalid borrower") {
            ...
            participant::borrower::State::ReceivingEscrowSignature { .. } => {
                BorrowerState::AwaitingTxSignatures
            }
    ...
    pub fn recover_transaction(&self) -> Result<String, JsValue> {
        match self.state.as_ref().expect("use of invalid borrower") {
            participant::borrower::State::SignaturesVerified(state) => Ok(
                bitcoin::consensus::encode::serialize_hex(state.recover_tx()),
            ),
    ...
    pub fn cancel_prefund(
        ...
        let fee_rate =
            bitcoin::FeeRate::from_sat_per_vb(fee_rate_sat_per_vb).ok_or("fee rate too high")?;
        let transactions = transactions
            .iter()
            .map(|tx| {
                let tx_bytes =
                    Vec::from_hex(&tx.as_string().unwrap()).map_err(into_debug_string)?;
